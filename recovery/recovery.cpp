...
main(int argc, char **argv) {
	...
	//add ben
	mknod("/dev/ttymxc0", S_IFCHR | 0600, (1 << 8) | 11);
    freopen("/dev/ttymxc0", "a", stdout); setbuf(stdout, NULL);
    freopen("/dev/ttymxc0", "a", stderr); setbuf(stderr, NULL);
    unlink("/dev/ttymxc0");
...
force:
    ...
    if (update_package != NULL) {
		...
		if (status != INSTALL_SUCCESS && force) {
		ui->ShowText(true);
		}
	...
	ui->Print("\n");
	ui->Print("------The OTA Upgrading is completed successfully------");
	ui->Print("\n");
