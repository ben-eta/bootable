...
#define LVDS_LBK_EN IMX_GPIO_NR(6, 15)
#define LVDS_EN_VDD IMX_GPIO_NR(6, 16)
/* [Evan],rev20160708, define gpio of pcie lan pwr */
#define LAN2_PWR_EN	IMX_GPIO_NR(6, 11)
...
static void setup_uart(void)
{
	...
	//add ben
	//sd3_cd
	mxc_iomux_v3_setup_pad(MX6Q_PAD_SD3_DAT5__GPIO_7_0);
}
...
//add ben
void lcd_pins_setting(void)
{
        mxc_iomux_v3_setup_pad(MX6X_IOMUX(PAD_NANDF_CS2__GPIO_6_15));
        mxc_iomux_v3_setup_pad(MX6X_IOMUX(PAD_NANDF_CS3__GPIO_6_16));

        /* LVDS_LBK_EN */
        gpio_direction_output(LVDS_LBK_EN, 0);

        /* LVDS_EN_VDD */
        gpio_direction_output(LVDS_EN_VDD, 0);
}

/*[Evan],rev20160708 */
void lan2_pwr_ctrl(void)
{
        /* LAN2_PWR_EN */
	mxc_iomux_v3_setup_pad(MX6X_IOMUX(PAD_NANDF_CS0__GPIO_6_11));
	gpio_direction_output(LAN2_PWR_EN, 0);
	udelay(1000*500*6);   //delay 3 seconds
	
	gpio_direction_output(LAN2_PWR_EN, 1);
	udelay(1000*500);	
}
...
*could not be power off anymore*/
	...
	//[Evan],rev20160708, pull low of pcie pwr
	lan2_pwr_ctrl();

#if defined CONFIG_MX6Q
        mxc_iomux_v3_setup_pad(MX6Q_PAD_GPIO_5__GPIO_1_5);// add zht
        reg = readl(GPIO1_BASE_ADDR + GPIO_GDIR);
    	reg |= (1 << 5) ;
   	writel(reg, GPIO1_BASE_ADDR + GPIO_GDIR);
   	...
   	reg= readl(GPIO1_BASE_ADDR + GPIO_DR);
        ...
        writel(reg, GPIO1_BASE_ADDR + GPIO_DR);
#elif defined CONFIG_MX6DL
        mxc_iomux_v3_setup_pad(MX6DL_PAD_GPIO_5__GPIO_1_5);// add bingking for solo
        reg = readl(GPIO1_BASE_ADDR + GPIO_GDIR);
        reg |= (1 << 5) ;
        writel(reg, GPIO1_BASE_ADDR + GPIO_GDIR);
