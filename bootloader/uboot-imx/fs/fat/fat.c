...
//add ben
long
do_fat_read_name (const char *filename, void *buffer, unsigned long maxsize,
	     int dols)
{
	char fnamecopy[2048];
	boot_sector bs;
				...
				if ((dentptr->attr & ATTR_VFAT) &&
					...
					if (dols == LS_ROOT) {
						...
						if (doit) {
							//add ben
							printf("tt1 \n");
							if (strcmp(l_name, "update.zip") == 0)
							{
								printf("update.zip return 10");
								return 100;
							}	
